...
**<예제 2-3>**
    ...
    B. double형이 float보다 더 높은 정밀도와 큰 표현 범위를 가지고 있는데도 왜 오차가 생길까?

    A. 여전히 제한된 정밀도를 가지고 있어 근사값만을 표현하기 때문일 것이다. 그래서 float이나 double은 정확한 소수 계산을 하는 decimal과 달리 과학적 계산이나 대용량 데이터 처리에 더 적합하다고 한다.
