...
같은 integer/short라도 바이트 수가 다르다

범위를 벗어나면 overflow(허용 범위를 넘는 값을 변수에 저장할 때 전혀 다른 값이 저장되는 현상)

math.h의 역할 어떤 함수 중 pow가 어디에 있는지

-power functions에 pow(returns base power raise to power exponent)
...
예재 2-3은 실수형 변수에 대하여, float를 찍었을 때 자릿수가 어떻게 나오는지 확인

float에서 정수형과 실수형을 신경 써서 

기본 자료형의 출력값 조정

폭을 나타내고 싶을 때 “%nd”로 표현
...
\n \r은 꼭 알아둬야 할 표현, 특수문자는 자주 쓰니 잘 외워두기

p71 1-2번 정리노트에 업로드, 문제 핵심

1 - char→int

2 - shortX double, constX 

/코드는 안 올려도 O 과정을 담은 노트 업로드
...
작은 범위에서 큰 범위로 변환을 해야 자료의 손실을 막을 수 있음
